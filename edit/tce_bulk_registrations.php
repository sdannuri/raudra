...
function F_db_connect($host = 'localhost', $port = '3306', $username = 'root', $password = 'ramesh123', $database = 'tcexam') {
...
function sendmailer($to,$username,$pass){
    ...
    $transport = Swift_SmtpTransport::newInstance('smtp.gmail.com', 465, "ssl")
        ->setUsername('brahma0545@gmail.com ')
        ->setPassword('susubran');

    $mailer = Swift_Mailer::newInstance($transport);

    $message = "<h2>Its Starts today</h2>";
    $message .="<p>Hi,</br> Please find your credentials for the test series GateExcel-135 below</p>";
    $message .="<ul>
                <li>User Name: ".$username."</li>
                <li>Password: ".$pass."</li>
                </ul>";
    $message.="<p> you can login to your account with this credentials and change your password. username can't be changed </p>";
...
//    $original_password = getPasswordHash($user_password);
    ...
    $sql="SELECT user_id FROM ".K_TABLE_USERS." WHERE user_password='".getPasswordHash($newpassword)."'";
    echo "<h5>" . $sql . K_NEWLINE . "</h5>";
    $r=mysql_query($sql,$db);

    while ($r and $row = mysql_fetch_assoc($r)) {
        echo "<h5>" . $row['user_id'] . K_NEWLINE . "</h5>";
    }
